...
class AutoRiaSpider(scrapy.Spider):
    ...
    def __init__(self, name=None, **kwargs):
        super(AutoRiaSpider, self).__init__(name, **kwargs)

        assert kwargs.get('city')
        assert kwargs.get('state')

        self.city = kwargs['city']
        self.state = kwargs['state']

        self._PARSE_URL = self.PARSE_URL % {"city": self.city,
                                            "state": self.state}

        self._REFERER_URL = self.REFERER_URL % {"city": self.city,
                                                "state": self.state}

    def parse(self, response):
        yield Request(
            url=self._PARSE_URL.format(page=0, rnd=random.randint(0, 2000000)),
    ...
    def search_items(self, response):
        ...
        headers['Referer'] = self._REFERER_URL.format(page=0)

        yield Request(url=self._PARSE_URL.format(page=page_num + 1,
    ...
    def parse_item(self, response):
        ...
        if self.allowed_models and \
                not any(model in title for model in self.allowed_models):
            self.log(u'It\'s not we are looking for: {}'.format(title))
            return

        if self.exclude_models and \
                any(model in title for model in self.exclude_models):
